...
<body>
	...
	<div class="row content">
		...
		<div class="column left">
			...
			<?php
      	// Subtitle
				$subtitle = get_post_meta( $post_id, '_impact_report_subtitle', true );
				if ( $subtitle ) {
					echo '<h3>' . esc_html( $subtitle ) . '</h3>';
				}

				// By the Numbers
				$numbers = get_post_meta( $post_id, '_impact_report_numbers', true );
				if ( $numbers ) {
		...
		<div class="column right">
			<?php
      	// Headline
				$headline = get_post_meta( $post_id, '_impact_report_headline', true );
				if ( $headline ) {
				...
				// Issue
				$issue = get_post_meta( $post_id, '_impact_report_issue', true );
				if ( $issue ) {
					echo '<h4>Issue</h4>';
				...
				// Response
				$response = get_post_meta( $post_id, '_impact_report_response', true );
				...
				$impacts_start = get_post_meta( $post_id, '_impact_report_impacts_position', true );
				$impacts = get_post_meta( $post_id, '_impact_report_impacts', true );
	...
	<div class="page-one row footer">
		...
		<?php
			...
			$front_bottom_left = get_post_meta( $post_id, '_impact_report_image_1', true );
		...
		<div class="column right">
			<?php
				// First page footer
				$footer_front = get_post_meta( $post_id, '_impact_report_footer_front', true );
				if ( $footer_front ) {
	...
	<div class="page-two row header">
		...
		<div class="column left">
      	...
      	// Second page top left image
				$back_top_left = get_post_meta( $post_id, '_impact_report_image_2', true );
				if ( $back_top_left ) {
		...
		<div class="column right">
      	...
      	// Second page top center image
				$back_top_center = get_post_meta( $post_id, '_impact_report_image_3', true );
				if ( back_top_center ) {
				...
				$back_top_right = get_post_meta( $post_id, '_impact_report_image_4', true );
				if ( $back_top_right ) {
	...
	<div class="row content">
		...
		<div class="column left">
			<?php
      	// Quotes
				$quotes = get_post_meta( $post_id, '_impact_report_quotes', true );
				if ( $quotes ) {
					echo '<h4>Quotes</h4>';
				...
				// Additional
				$additional_title = get_post_meta( $post_id, '_impact_report_additional_title', true );
				$additional = get_post_meta( $post_id, '_impact_report_additional', true );
	...
	<div class="page-two row footer">
		<?php
			...
			$back_bottom_left = get_post_meta( $post_id, '_impact_report_image_5', true );
			if ( $back_bottom_left ) {
			...
			// Second page footer
			$footer_back = get_post_meta( $post_id, '_impact_report_footer_back', true );
			if ( $footer_back ) {
